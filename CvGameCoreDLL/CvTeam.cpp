	...
	for (iI = 0; iI < MAX_TEAMS; iI++)
	{
		if ((iI != GetID()) && (iI != eTeam))
		{
			if (GET_TEAM((TeamTypes)iI).isAlive())
			{
				...
				{
					GET_TEAM(eTeam).meet((TeamTypes)iI);
				}
			}
		}
	}
...
void CvTeam::processBuilding(BuildingTypes eBuilding, int iChange, bool bFirst)
{
	...
	if (bFirst && iChange > 0)
	{
		...
		if (pBuildingInfo->IsDiplomaticVoting())
		{
			...
			GC.getGame().DoResetUnitedNationsCountdown();
		}
	}
...
//	--------------------------------------------------------------------------------
/// Is it time to send out a "help the Minors" reminder when they're fighting a Warmonger?
void CvTeam::DoTestWarmongerReminder()
{
	if (IsMinorCivWarmonger())
	{
		...
		const char* strLeaderNameKey = GET_PLAYER(getLeaderID()).getCivilizationShortDescriptionKey();

		int iOtherPlayer;

		int iShortestNumTurnsAtWar = INT_MAX;

		// Loop through all Minors to see if we've been at war with them for 2-ish turns
		TeamTypes eMinorTeam;
		for (int iMinorTeam = 0; iMinorTeam < MAX_CIV_TEAMS; iMinorTeam++)
		{
			eMinorTeam = (TeamTypes) iMinorTeam;

			if (GET_TEAM(eMinorTeam).isMinorCiv())
			{
				if (isAtWar(eMinorTeam))
				{
					if (GetNumTurnsAtWar(eMinorTeam) < iShortestNumTurnsAtWar)
					{
						iShortestNumTurnsAtWar = GetNumTurnsAtWar(eMinorTeam);
					}
				}
			}
		}

		// Notification for all other players in the game
		if (iShortestNumTurnsAtWar == /*2*/ GC.getTXT_KEY_MINOR_GIFT_UNITS_REMINDER())
		{
			PlayerTypes eOtherPlayer;
			for (iOtherPlayer = 0; iOtherPlayer < MAX_MAJOR_CIVS; iOtherPlayer++)
			{
				eOtherPlayer = (PlayerTypes) iOtherPlayer;

				if (GET_PLAYER(eOtherPlayer).isAlive())
				{
					// Has this player's team met the guy attacking the minors?
					if (GET_TEAM(GET_PLAYER(eOtherPlayer).getTeam()).isHasMet(GetID()))
					{
						pNotifications = GET_PLAYER(eOtherPlayer).GetNotifications();
						if (pNotifications != NULL)
						{
							strMessage = Localization::Lookup("TXT_KEY_NOTIFICATION_MINOR_WARMONGER_REMINDER");
							strMessage << strLeaderNameKey;
							strSummary = Localization::Lookup("TXT_KEY_NOTIFICATION_SUMMARY_MINOR_WARMONGER_REMINDER");
							strSummary;
							pNotifications->Add(NOTIFICATION_WAR, strMessage.toUTF8(), strSummary.toUTF8(), -1, -1, -1);
						}
					}
				}
			}
...
//	-----------------------------------------------------------------------------------------------
void CvTeam::DoDeclareWar(TeamTypes eTeam, bool bDefensivePact)
{
	...
	if (!isBarbarian())
	{
		...
		// Reset Defensive Pact status for the other team
	...
	if (!isBarbarian())
	{
		...
		if (!GET_TEAM(eTeam).isMinorCiv() && GetTurnMadePeaceTreatyWithTeam(eTeam) != -1)
			...
			if (GET_PLAYER(eMinor).getTeam() == GetID())
			{
				if (GET_PLAYER(eMinor).isAlive())
				{
					if (bWar)
						...
						GET_PLAYER(eMinor).GetMinorCivAI()->DoNowPeaceWithTeam();
				}
			}
		}
	}

	// If someone has a Player for an ally, then they join up
	if (bWar)
	{
		PlayerTypes eMinor;
		PlayerTypes ePlayer;
		for (int iPlayerCivLoop = 0; iPlayerCivLoop < MAX_MAJOR_CIVS; iPlayerCivLoop++)
		{
			...
			// Loop through minors to see if any of them are allies of this player
			for (int iMinorCivLoop = MAX_MAJOR_CIVS; iMinorCivLoop < MAX_CIV_PLAYERS; iMinorCivLoop++)
			{
				eMinor = (PlayerTypes) iMinorCivLoop;
				...
				if (GET_PLAYER(eMinor).GetMinorCivAI()->IsAllies(ePlayer))
				{
					...
					{
						// Match war state
						GET_TEAM(GET_PLAYER(eMinor).getTeam()).DoDeclareWar(eTeam, false);
					}
				}
			}
		}
...
{
	DoMakePeace(eTeam, bBumpUnits);
}

//	------------------------------------------------------------------------------------------------
//	The make peace handler, can be called recursively
void CvTeam::DoMakePeace(TeamTypes eTeam, bool bBumpUnits)
{
	CvString strBuffer;
	int iI;
						...
						if (GET_PLAYER(eOurMinor).GetMinorCivAI()->GetAlly() == eOurPlayer)
						{
							...
							if (!bPeaceBlocked)
							{
								if (!GET_PLAYER(eOurMinor).GetMinorCivAI()->IsPermanentWar(eTeamWeMadePeaceWith))
									GET_TEAM(GET_PLAYER(eOurMinor).getTeam()).DoMakePeace(eTeamWeMadePeaceWith, true);
							}
						}
					}
				}
...
//	--------------------------------------------------------------------------------
void CvTeam::meet(TeamTypes eTeam)
{
	if (!isHasMet(eTeam))
	{
		makeHasMet(eTeam);
		GET_TEAM(eTeam).makeHasMet(GetID());
...
{
	// By default, player votes for himself
	TeamTypes eVoteTeam = GetID();

	// Minor civs vote for their favored nation
	if (isMinorCiv())
	{
	...
	// If this team was liberated then they automatically vote for who liberated them
...
int CvTeam::GetTotalSecuredVotes() const
{
	...
	for (int iTeamLoop = 0; iTeamLoop < MAX_CIV_TEAMS; iTeamLoop++)
	{
		...
		{
			if (GET_TEAM(eLoopTeam).GetTeamVotingForInDiplo() == GetID())
			{
				iTotalVotes++;

				if (GET_TEAM(eLoopTeam).IsHomeOfUnitedNations())
					iTotalVotes += /*1*/ GC.getOWN_UNITED_NATIONS_VOTE_BONUS();
			}
		}
	}
...
void CvTeam::SetNumMinorCivsAttacked(int iValue)
{
	...
	{
		bool bWasAggressor = IsMinorCivAggressor();
		bool bWasWarmonger = IsMinorCivWarmonger();
		...
		// Now a warmonger
		if (!bWasWarmonger && IsMinorCivWarmonger())
		{
			// Notification for all other players in the game
			PlayerTypes eOtherPlayer;
			for (int iOtherPlayer = 0; iOtherPlayer < MAX_MAJOR_CIVS; iOtherPlayer++)
			{
				...
				{
					// Not this guy
					if (GetID() != GET_PLAYER(eOtherPlayer).getTeam())
					{
						// Has this player's team met the guy attacking the minors?
						if (GET_TEAM(GET_PLAYER(eOtherPlayer).getTeam()).isHasMet(GetID()))
						{
							...
							if (pNotifications != NULL)
							{
								strMessage = Localization::Lookup("TXT_KEY_NOTIFICATION_MINOR_WARMONGER_OTHER");
								strMessage << strLeaderNameKey;
								strSummary = Localization::Lookup("TXT_KEY_NOTIFICATION_SUMMARY_MINOR_WARMONGER_OTHER");
								strSummary << strLeaderNameKey;
								pNotifications->Add(NOTIFICATION_MINOR_QUEST, strMessage.toUTF8(), strSummary.toUTF8(), -1, -1, -1);
							}
						}
					}
				}
			...
			strMessage = Localization::Lookup("TXT_KEY_NOTIFICATION_MINOR_WARMONGER");
			strSummary = Localization::Lookup("TXT_KEY_NOTIFICATION_SUMMARY_MINOR_WARMONGER");
			bShowMessage = true;
		}
		// Now an aggressor
		else if (!bWasAggressor && IsMinorCivAggressor())
		{
			strMessage = Localization::Lookup("TXT_KEY_NOTIFICATION_MINOR_AGGRESSOR");
			strSummary = Localization::Lookup("TXT_KEY_NOTIFICATION_SUMMARY_MINOR_AGGRESSOR");
			bShowMessage = true;
		}
		...
		if (bShowMessage)
		{
			...
			for (int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
			{
				...
				if (GET_PLAYER(ePlayer).getTeam() == GetID())
				{
					...
					pNotifications->Add(NOTIFICATION_MINOR, strMessage.toUTF8(), strSummary.toUTF8(), -1, -1, -1);
				}
			}
		}
	}
...
//	--------------------------------------------------------------------------------
void CvTeam::makeHasMet(TeamTypes eIndex)
{
	int iI;
	...
	if (!isHasMet(eIndex))
	{
		...
		if (GET_TEAM(eIndex).isMinorCiv())
		{
			if (!isAtWar(eIndex))
			{
				...
				if (GET_PLAYER(GET_TEAM(eIndex).getLeaderID()).getCapitalCity() != NULL)
				{
					iCapitalX  = pCap->getX();
					iCapitalY  = pCap->getY();
					iCapitalID = pCap->GetID();

					GC.getMap().plotUnchecked(iCapitalX, iCapitalY)->setRevealed(GetID(), true);
				}

				// Meet the Minor
				if (!isMinorCiv())
				{
					GET_PLAYER(GET_TEAM(eIndex).getLeaderID()).GetMinorCivAI()->DoFirstContactWithMajor(GetID());
				}
...
void CvTeam::setHasTech(TechTypes eIndex, bool bNewValue, PlayerTypes ePlayer, bool bFirst, bool bAnnounce)
{
	...
	if (GetTeamTechs()->HasTech(eIndex) != bNewValue)
	{
				...
				if (GC.getGame().countKnownTechNumTeams(eIndex) == 1)
				{
					if (pkTechInfo->GetFirstFreeUnitClass() != NO_UNITCLASS)
					{
						...
						if (eFreeUnit != NULL)
						{
							...
							{
								pCapitalCity->GetCityCitizens()->DoSpawnGreatPerson(eFreeUnit);
							}
						}
					}
		...
		for (int iPlayerLoop = 0; iPlayerLoop < MAX_MAJOR_CIVS; iPlayerLoop++)
		{
			if (GET_PLAYER((PlayerTypes) iPlayerLoop).getTeam() == GetID())
			{
...
void CvTeam::Read(FDataStream& kStream)
{
	...
	if(uiVersion >= 10)
	{
		...
		kExtraMovesWrapper.readCounted(kStream, 0);
	}
	else
	{
		int iOldNumDomainTypes = 4; // Before version 10, NUM_DOMAIN_TYPES was 4
	...
	{
		ArrayWrapper<int> kUnitClassCountWrapper(GC.getNumUnitClassInfos(), m_paiUnitClassCount);
	...
	if (uiVersion >= 6)
	{
		ImprovementArrayHelpers::ReadYieldArray(kStream, m_ppaaiImprovementYieldChange);
		ImprovementArrayHelpers::ReadYieldArray(kStream, m_ppaaiImprovementNoFreshWaterYieldChange);
		ImprovementArrayHelpers::ReadYieldArray(kStream, m_ppaaiImprovementFreshWaterYieldChange);
	}
	else
	{
		for (int i = 0; i < 20; ++i)
		{
			ArrayWrapper<int> kImprovementYieldChangeWrapper(NUM_YIELD_TYPES, m_ppaaiImprovementYieldChange[i]);
			kStream >> kImprovementYieldChangeWrapper;

			if (uiVersion >= 2)
			{
				ArrayWrapper<int> kImprovementNoFreshWaterWrapper(NUM_YIELD_TYPES, m_ppaaiImprovementNoFreshWaterYieldChange[i]);
			...
			ArrayWrapper<int> ImprovementFreshWaterWrapper(NUM_YIELD_TYPES, m_ppaaiImprovementFreshWaterYieldChange[i]);
...
void CvTeam::Write(FDataStream& kStream) const
{
	...
	ArrayWrapperConst<int> kExtraMovesWrapper(NUM_DOMAIN_TYPES, &m_aiExtraMoves[0]);
	kExtraMovesWrapper.writeCounted(kStream);
