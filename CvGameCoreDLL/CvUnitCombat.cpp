...
void CvUnitCombat::ResolveMeleeCombat(const CvCombatInfo &kCombatInfo, uint uiParentEventID)
{
	...
	if (pkAttacker)
	{
		...
		else
		{
			if (pkTargetPlot)
			{
				bool bCanAdvance = pkTargetPlot->getNumVisibleEnemyDefenders(pkAttacker) == 0 ? true : false;
				if (bCanAdvance)
				{
...
void CvUnitCombat::GenerateRangedCombatInfo(CvUnit& kAttacker, CvUnit *pkDefender, CvPlot &plot, CvCombatInfo *pkCombatInfo)
{
	...
	else
	{
		...
		if (iDamage + pCity->getDamage() >= GC.getMAX_CITY_HIT_POINTS())
		{
			iDamage = GC.getMAX_CITY_HIT_POINTS() - pCity->getDamage() - 1;
		}
			...
			if (pkDefender)
			{
				if (pkAttacker)
				{
					...
					if (iDamage + pkDefender->getDamage() >= GC.getMAX_HIT_POINTS())
					{
						...
						kAttackingPlayer.ReportCultureFromKills(pkDefender->getX(), pkDefender->getY(), pkDefender->GetBaseCombatStrength(true), pkDefender->isBarbarian());
						kAttackingPlayer.ReportGoldFromKills(pkDefender->getX(), pkDefender->getY(), pkDefender->GetBaseCombatStrength(true));
					}
...
void CvUnitCombat::ResolveCityMeleeCombat(const CvCombatInfo& kCombatInfo, uint uiParentEventID)
{
	...
	if (pkAttacker && pkDefender)
	{
		if (pkAttacker->isBarbarian() && (pkDefender->getDamage() >= GC.getMAX_CITY_HIT_POINTS()))
		{
			// 1 HP left
			pkDefender->setDamage(GC.getMAX_CITY_HIT_POINTS() - 1);
		...
		// City conquest
		else if (pkDefender->getDamage() >= GC.getMAX_CITY_HIT_POINTS())
		{
...
void CvUnitCombat::GenerateAirCombatInfo(CvUnit& kAttacker, CvUnit *pkDefender, CvPlot &plot, CvCombatInfo *pkCombatInfo)
{
	...
	else
	{
		...
		// Cities can't be knocked to less than 1 HP
		if (iAttackerDamageInflicted + pCity->getDamage() >= GC.getMAX_CITY_HIT_POINTS())
		{
			iAttackerDamageInflicted = GC.getMAX_CITY_HIT_POINTS() - pCity->getDamage() - 1;
		}
	...
	for (int i = 0; i < iDamageMembers; ++i)
	{
		...
		if (kEntry.IsCity())
		{
			...
			if (pkCity)
			{
				...
				if (kEntry.GetFinalDamage() >= GC.getMAX_CITY_HIT_POINTS() && !pkCity->IsOriginalCapital())
				{
		...
		for (int iDY = -(iBlastRadius); iDY <= iBlastRadius; iDY++)
		{
			...
			if (pLoopPlot != NULL)
			{
				...
				if (pLoopCity != NULL)
				{
					...
					if (bKillCity)
					{
						iTotalDamage = GC.getMAX_CITY_HIT_POINTS();
					}
					else
					{
						// Add damage to the city
						iTotalDamage = (/*25*/ GC.getMAX_CITY_HIT_POINTS() - pLoopCity->getDamage()) * /*50*/ GC.getNUKE_CITY_HIT_POINT_DAMAGE();
						iTotalDamage /= 100;

						iTotalDamage += pLoopCity->getDamage();

						// Can't bring a city below 1 HP
						iTotalDamage = min(iTotalDamage, /*25*/ GC.getMAX_CITY_HIT_POINTS() - 1);
					}
					...
					if (pkDamageEntry)
					{
						...
						pkDamageEntry->SetMaxHitPoints(GC.getMAX_CITY_HIT_POINTS());
...
void CvUnitCombat::ApplyPostCombatTraitEffects(CvUnit *pkWinner, CvUnit *pkLoser)
{
	...
	// Earn bonuses for kills?
	kPlayer.ReportCultureFromKills(pkLoser->getX(), pkLoser->getY(), pkLoser->GetBaseCombatStrength(true), pkLoser->isBarbarian());
	kPlayer.ReportGoldFromKills(pkLoser->getX(), pkLoser->getY(), pkLoser->GetBaseCombatStrength(true));
