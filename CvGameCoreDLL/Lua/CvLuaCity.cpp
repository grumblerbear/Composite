...
void CvLuaCity::PushMethods(lua_State* L, int t)
{
	...
	Method(GetJONSCulturePerTurnFromTerrain);
	Method(ChangeJONSCulturePerTurnFromTerrain);
	...
	//Method(IsCitizensAutomated);
	//Method(SetCitizensAutomated);
...
//------------------------------------------------------------------------------
// int IsCanPurchase(UnitTypes eUnitType, BuildingTypes eBuildingType, ProjectTypes eProjectType);
int CvLuaCity::lIsCanPurchase(lua_State* L)
{
	CvCity* pkCity = GetInstance(L);
	const bool bOnlyTestVisible = lua_toboolean(L, 2);
	const UnitTypes eUnitType = (UnitTypes) lua_tointeger(L, 3);
	const BuildingTypes eBuildingType = (BuildingTypes) lua_tointeger(L, 4);
	const ProjectTypes eProjectType = (ProjectTypes) lua_tointeger(L, 5);

	const bool bResult = pkCity->IsCanPurchase(bOnlyTestVisible, eUnitType, eBuildingType, eProjectType);
...
int CvLuaCity::lGetJONSCulturePerTurnFromTerrain(lua_State* L)
{
	return BasicLuaMethod(L, &CvCity::GetJONSCulturePerTurnFromTerrain);
}
//------------------------------------------------------------------------------
//void ChangeJONSCulturePerTurnFromTerrain(int iChange);
int CvLuaCity::lChangeJONSCulturePerTurnFromTerrain(lua_State* L)
{
	return BasicLuaMethod(L, &CvCity::ChangeJONSCulturePerTurnFromTerrain);
}
//------------------------------------------------------------------------------
...
int CvLuaCity::lGetHappiness(lua_State* L)
{
	...
	int iHappines = pkCity->GetHappinessFromBuildings();

	//Create a filtered list of valid policies so that we don't have to do this for each building.
	std::vector<CvPolicyEntry*> kValidPolicies;
	kValidPolicies.reserve(numPolicyInfos);			//Allocate Worst case
	for (int iPolicyLoop = 0; iPolicyLoop < numPolicyInfos; iPolicyLoop++)
	{
		const PolicyTypes ePolicy = static_cast<PolicyTypes>(iPolicyLoop);
		CvPolicyEntry* pkPolicyInfo = GC.getPolicyInfo(ePolicy);
		if(pkPolicyInfo)
		{
			if (pkPlayerPolicies->HasPolicy(ePolicy) && !pkPlayerPolicies->IsPolicyBlocked(ePolicy))
			{
				kValidPolicies.push_back(pkPolicyInfo);
			}
		}
	}

	for(int iBuildingLoop = 0; iBuildingLoop < numBuildingInfos; ++iBuildingLoop)
	{
		const BuildingTypes eBuilding = static_cast<BuildingTypes>(iBuildingLoop);
		CvBuildingEntry* pkBuilding = GC.getBuildingInfo(eBuilding);
		if(pkBuilding)
		{
			const int numBuildingsOfType = pkCityBuildings->GetNumBuilding(eBuilding);
			if(numBuildingsOfType > 0)
			{
				const BuildingClassTypes eBuildingClass = static_cast<BuildingClassTypes>(pkBuilding->GetBuildingClassType());

				for(std::vector<CvPolicyEntry*>::iterator it = kValidPolicies.begin(); it != kValidPolicies.end(); ++it)
				{
					CvPolicyEntry* pkPolicyInfo = (*it);
					const int buildingClassHappiness = pkPolicyInfo->GetBuildingClassHappiness(eBuildingClass);
					if (buildingClassHappiness != 0)
					{
						iHappines += numBuildingsOfType * buildingClassHappiness;
					}
				}
			}
		}
