...
//------------------------------------------------------------------------------
const int g_CurrentCvPlayerVersion = 48;
...
void CvPlayer::acquireCity(CvCity* pOldCity, bool bConquest, bool bGift)
{
	...
	bool bIsMinorCivBuyout = (pOldCity->GetPlayer()->isMinorCiv() && bGift && GetPlayerTraits()->IsAbleToAnnexCityStates()); // Austria UA
...
#pragma warning ( disable : 6011 ) 
	...
	for(iI = 0; iI < GC.getNumBuildingInfos(); iI++)
	{
		...
		if(pkLoopBuildingInfo)
		{
			...
			{
				changeFreeBuildingCount(eTraitFreeBuilding, 1);
			}
	...
	else //if (bConquest)
	{
		...
		if(lResult == 0)
		{
			...
			else if(!GC.getGame().isOption(GAMEOPTION_NO_HAPPINESS))
			{
				...
				// If this city wasn't originally ours, or if it is a minor civ buyout, give the human the choice to annex or puppet it
				if(pNewCity->getOriginalOwner() != GetID() || bIsMinorCivBuyout)
				{
					if(GC.getGame().getActivePlayer() == GetID())
					{
						CvPopupInfo kPopupInfo(BUTTONPOPUP_CITY_CAPTURED, pNewCity->GetID(), iCaptureGold, iCaptureCulture, eLiberatedPlayer, bIsMinorCivBuyout);
			...
			else if(iCaptureGold > 0)
			{
				...
				{
					strBuffer = GetLocalizedText("TXT_KEY_POPUP_GOLD_AND_CULTURE_CITY_CAPTURE", iCaptureGold, iCaptureCulture, pNewCity->getNameKey());
					GC.GetEngineUserInterface()->AddCityMessage(0, pNewCity->GetIDInfo(), GetID(), true, GC.getEVENT_MESSAGE_TIME(), strBuffer);
...
void CvPlayer::receiveGoody(CvPlot* pPlot, GoodyTypes eGoody, CvUnit* pUnit)
{
	...
	if(kGoodyInfo.isUpgradeUnit())
	{
		...
		if(eUpgradeUnit != NO_UNIT)
		{
			...
			CvUnit* pNewUnit = initUnit(eUpgradeUnit, pPlot->getX(), pPlot->getY(), newAIDefault, NO_DIRECTION, false, false);
			pUnit->finishMoves();
...
//	--------------------------------------------------------------------------------
/// Handle earning a yield from combat wins
void CvPlayer::ReportYieldFromKills(YieldTypes eYield, UnitTypes eAttackingUnitType, int iX, int iY, int iStrength, bool bWasBarbarian)
{
	int iValue = 0;
	...
	if(iStrength > 0)
	{
		...
		{
			CvUnitEntry* pkUnitInfo = GC.getUnitInfo(eAttackingUnitType);
			if(pkUnitInfo)
			{
				iValue += pkUnitInfo->GetYieldFromKills(eYield);
			}
		}

		iValue = (iValue * iStrength) / 100;
		if(iValue > 0)
		{
			switch(eYield)
			{
			...
			case YIELD_CULTURE:
				...
				colorString = "[COLOR_MAGENTA]+%d[ENDCOLOR]";
				break;

			case YIELD_FAITH:
				ChangeFaith(iValue);
				colorString = "[COLOR_WHITE]+%d[ENDCOLOR]";
				break;
			...
			case YIELD_SCIENCE:
				...
				colorString = "[COLOR_BLUE]+%d[ENDCOLOR]";
				break;
			}
			...
			if(GetID() == GC.getGame().getActivePlayer())
			{
				...
				sprintf_s(text, colorString, iValue);
...
int CvPlayer::GetHappinessFromReligion()
{
	...
	if(eFoundedReligion != NO_RELIGION)
	{
		...
		if(pReligion)
		{
			...
			int iHappinessPerFollowingCity = pReligion->m_Beliefs.GetHappinessPerFollowingCity();
			iHappinessFromReligion += (pReligions->GetNumCitiesFollowing(eFoundedReligion) * iHappinessPerFollowingCity);
			...
			if(bValue)
			{
				...
				for(uint ui = 0; ui < MAX_MAJOR_CIVS; ui++)
				{
					...
					if(ePlayer == GetID())
					{
						...
						else if (GET_TEAM(GET_PLAYER(ePlayer).getTeam()).isHasMet(eWinningTeam))
						{
...
void CvPlayer::AddIncomingUnit(PlayerTypes eFromPlayer, CvUnit* pUnit)
{
	...
	if(isMinorCiv() && eFromPlayer < MAX_MAJOR_CIVS)
	{
		...
		int iGPInfluence = GET_PLAYER(eFromPlayer).GetPlayerTraits()->GetGreatPersonGiftInfluence();
		if(iGPInfluence > 0 && pUnit->IsGreatPerson())
		{
			// Doesn't increment count either
			GetMinorCivAI()->ChangeFriendshipWithMajor(eFromPlayer, iGPInfluence);
		}

		else if(GetMinorCivAI()->GetNumUnitsGifted(eFromPlayer) < /*3*/ GC.getMAX_INFLUENCE_FROM_MINOR_GIFTS())
		{
...
int CvPlayer::getNumResourceTotal(ResourceTypes eIndex, bool bIncludeImport) const
{
	...
	if (pkResourceInfo)
	{
		if(GC.getResourceInfo(eIndex)->getResourceUsage() == RESOURCEUSAGE_STRATEGIC)
		{
...
void CvPlayer::Read(FDataStream& kStream)
{
	...
	if(uiVersion >= 27 && uiVersion < 32)
	{
		int temp;
		kStream >> temp; // Obsolete for m_iFaithLastGreatProphet;
	}
	...
	kStream >> m_iHappinessFromGarrisonedUnits;  //unused
	...
	else
	{
		...
		int iNumEntries;
		kStream >> iNumEntries;
		for(iYieldLoop = 0; iYieldLoop < iNumEntries; iYieldLoop++)
		{
			kStream >> iYield;
			m_aiCityYieldChange.setAt(iYieldLoop, iYield);
		}
		kStream >> iNumEntries;
		for(iYieldLoop = 0; iYieldLoop < iNumEntries; iYieldLoop++)
		{
			kStream >> iYield;
			m_aiCoastalCityYieldChange.setAt(iYieldLoop, iYield);
		}
		kStream >> iNumEntries;
		for(iYieldLoop = 0; iYieldLoop < iNumEntries; iYieldLoop++)
		{
			kStream >> iYield;
			m_aiCapitalYieldChange.setAt(iYieldLoop, iYield);
		}
	}
	...
	if(uiVersion >= 25)
	{
		...
		for(int iYieldLoop = 0; iYieldLoop < NUM_YIELD_TYPES; iYieldLoop++)
		{
			...
			m_aiCityYieldChange.setAt(eYieldLoop, m_aiCityYieldChange[eYieldLoop]);
			m_aiCapitalYieldChange.setAt(eYieldLoop, m_aiCapitalYieldChange[eYieldLoop]);
		}
	}
	else if(uiVersion < 13)
	{
		YieldTypes eYieldLoop;
		for(int iYieldLoop = 0; iYieldLoop < 4; iYieldLoop++)
		{
			eYieldLoop = (YieldTypes) iYieldLoop;
	...
	{
		for(int i = 0; i < MAX_PLAYERS; i++)
			m_aiMinorFriendshipAnchors.setAt(i, 0);
	}
	...
	if(uiVersion <= 19)
	{
		...
		CvInfosSerializationHelper::ReadV0DataArray(kStream, aTemp, GC.getNumResourceInfos(), CVINFO_V0_TAGS(CvInfosSerializationHelper::ms_V0ResourceTags));
		m_paiNumResourceUsed = aTemp;
		CvInfosSerializationHelper::ReadV0DataArray(kStream, aTemp, GC.getNumResourceInfos(), CVINFO_V0_TAGS(CvInfosSerializationHelper::ms_V0ResourceTags));
		m_paiNumResourceTotal = aTemp;
		CvInfosSerializationHelper::ReadV0DataArray(kStream, aTemp, GC.getNumResourceInfos(), CVINFO_V0_TAGS(CvInfosSerializationHelper::ms_V0ResourceTags));
		m_paiResourceGiftedToMinors = aTemp;
		CvInfosSerializationHelper::ReadV0DataArray(kStream, aTemp, GC.getNumResourceInfos(), CVINFO_V0_TAGS(CvInfosSerializationHelper::ms_V0ResourceTags));
		m_paiResourceExport = aTemp;
		CvInfosSerializationHelper::ReadV0DataArray(kStream, aTemp, GC.getNumResourceInfos(), CVINFO_V0_TAGS(CvInfosSerializationHelper::ms_V0ResourceTags));
		m_paiResourceImport = aTemp;
		CvInfosSerializationHelper::ReadV0DataArray(kStream, aTemp, GC.getNumResourceInfos(), CVINFO_V0_TAGS(CvInfosSerializationHelper::ms_V0ResourceTags));
		m_paiResourceFromMinors = aTemp;
	}
	else
	{
	...
	{
		// doing this because when it is initialized on load it skips the initialization because the player doesn't have a civilization type defined. Now it does so call the init!
		m_pEspionage->Init(this);
	}
	...
	else
	{
		...
		for(int i = 0; i <iNumEntries; ++i)
		{
			...
			newYield[4] = 0;
			newYield[5] = 0;
			m_ppaaiSpecialistExtraYield.setAt(i, newYield);
		}
		m_ppaaiImprovementYieldChange.clear();
		m_ppaaiImprovementYieldChange.resize(GC.getNumImprovementInfos());
		kStream >> iNumEntries;
		for(int i = 0; i < iNumEntries; ++i)
		{
			...
			newYield[4] = 0;
			newYield[5] = 0;
			m_ppaaiImprovementYieldChange.setAt(i, newYield);
		}
	}
